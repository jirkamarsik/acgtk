...
struct
  ...
  let rec content_to_string c =
    ...
    | Constr (i,lst) -> Printf.sprintf "Contructeur %d(%s)" i (Utils.string_of_list "," (fun j -> content_to_string(Link_to j)) lst)
  ...
  let to_string h =
	...
	| Constr (c,i_args) -> Printf.sprintf "%s%d\t\t--->\t(%s)\n" acc i (Utils.string_of_list " -> " string_of_int i_args))
      h.parents
      ""
