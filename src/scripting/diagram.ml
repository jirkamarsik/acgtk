...
let inline (|>) (x : 'a) (f : 'a -> 'b) : 'b =
  f x

(* The following definition does not admit inline... *)
let (>>) (f : 'a -> 'b) (g : 'b -> 'c) : 'a -> 'c =
  fun x -> g (f x)
...
let rotate (angle : float) : diagram -> diagram =
  transform (Matrix.init_rotate angle)
...
let line ((x1, y1) : point) ((x2, y2) : point) : diagram =
  let p = trace_path (fun cr ->
            move_to cr x1 y1;
            line_to cr x2 y2) in
  Draw (p, Stroke)
...
let text_path (opts : text_opts) : Path.t =
  trace_path (fun cr ->
    Font_face.set cr opts.face;
    set_font_size cr opts.size;
    move_to cr 0. 0.;
...
let text ?(face : toy_font = default_font) ?(size : float = 10.)
  ...
  Draw (text_path { contents = contents;
                    face = face;
                    size = size; },
        Fill)
  |> center
...
let rectangle_outline (width : float) (height : float) : diagram =
  Draw (path_of_rectangle (rect_at_orig width height), Stroke)
  |> center

let rectangle_full (width : float) (height : float) : diagram =
  Draw (path_of_rectangle (rect_at_orig width height), Fill)
  |> center
...
let pad ?(all : float = 0.)
  ...
  let wrapper = Draw (path_of_rectangle envelope, Fill)
...
let circle_outline (radius : float) : diagram =
  Draw (path_of_circle (0., 0.) radius, Stroke)

let circle_full (radius : float) : diagram =
  Draw (path_of_circle (0., 0.) radius, Fill)
...
let rec tighten_text (d : diagram) : diagram =
  ...
  | Draw (p, op) -> Draw (p, op)
  | Text opts -> Draw (text_path opts, Fill)
  
let show_origin (d : diagram) : diagram =
  blend [ circle_full 1.5
            |> color red;
          circle_outline 1.5
            |> color black
